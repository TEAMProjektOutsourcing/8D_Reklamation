...
if (file_exists(__DIR__ . '/claim_group_helper.php')) { require_once __DIR__ . '/claim_group_helper.php'; }
if (file_exists(__DIR__ . '/action_read_helper.php')) { require_once __DIR__ . '/action_read_helper.php'; }

$user = current_user();
$currentPage = basename($_SERVER['SCRIPT_NAME'] ?? '');

$myOpenActionsCount = ($user && function_exists('my_unread_open_action_count')) ? my_unread_open_action_count((int)$user['id']) : ($user ? my_open_action_count((int)$user['id']) : 0);
$myCriticalActionsCount = ($user && function_exists('my_unread_critical_action_count')) ? my_unread_critical_action_count((int)$user['id']) : ($user ? my_critical_action_count((int)$user['id']) : 0);
...
function can_view_analytics_nav(?array $user = null): bool
{
    if (!$user) {
        $user = current_user();
    }
...
<body>
<nav class="navbar navbar-expand-xl navbar-light app-navbar-modern sticky-top">
    <div class="container-fluid">
        <a class="navbar-brand app-brand" href="dashboard.php" aria-label="Reklamation8D Dashboard">
            <img
            ...
            <span class="app-brand-fallback">
                Reklamation8D
                <span class="app-brand-sub">8D Tool</span>
            </span>
        </a>

        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#mainNav" aria-controls="mainNav" aria-expanded="false" aria-label="Navigation öffnen">
        ...
        <div class="collapse navbar-collapse" id="mainNav">
            <?php if ($user): ?>
                <ul class="navbar-nav me-auto mb-2 mb-xl-0">
                    ...
                    <?php if (can_view_analytics_nav($user)): ?>
                    ...
                    <li class="nav-item">
                        <a class="nav-link position-relative<?= nav_active('my_actions.php', $currentPage) ?>" href="my_actions.php">
                            ...
                            <?php if ($myOpenActionsCount > 0): ?>
                                <span class="nav-action-badge badge rounded-pill <?= $myCriticalActionsCount > 0 ? 'bg-danger' : 'bg-primary' ?>" title="<?= (int)$myOpenActionsCount ?> ungelesene offene Maßnahme<?= $myOpenActionsCount === 1 ? '' : 'n' ?>">
                    ...
                    <?php if (can_edit()): ?>
                        <li class="nav-item<?= $myOpenActionsCount > 0 ? ' ms-xl-2' : '' ?>">
                            <a class="nav-link nav-cta<?= nav_active('claim_create.php', $currentPage) ?>" href="claim_create.php">+ Neue Reklamation</a>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php if (is_admin()): ?>
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item<?= nav_active_any(['users.php', 'user_form.php'], $currentPage) ?>" href="users.php">Benutzer</a></li>
                                <li><a class="dropdown-item<?= nav_active('locations.php', $currentPage) ?>" href="locations.php">Standorte</a></li>
                                <li><a class="dropdown-item<?= nav_active_any(['groups.php', 'group_form.php', 'group_save.php', 'group_delete.php'], $currentPage) ?>" href="groups.php">Gruppen</a></li>
                                <li><a class="dropdown-item<?= nav_active('action_escalation_run.php', $currentPage) ?>" href="action_escalation_run.php">Ampel-Eskalation</a></li>
                ...
                <?php if (is_admin() && $locationsEnabled && $allowedLocations): ?>
                    <form method="post" action="set_location.php" class="d-flex align-items-center me-xl-3 mb-2 mb-xl-0 location-switcher-form">
                        <?= csrf_field() ?>
                ...
                <?php elseif (is_admin() && $locationsEnabled): ?>
                    <a href="run_location_migration.php" class="btn btn-sm btn-warning me-xl-3 mb-2 mb-xl-0">Standorte zuweisen</a>
                <?php endif; ?>

                <div class="dropdown">
                    <button class="btn btn-sm dropdown-toggle app-user-btn" type="button" data-bs-toggle="dropdown">
